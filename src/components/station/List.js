	...
	onScroll(e) {
		...
		if (this._scrollDirection !== direction) {
			...
			if (direction === 'down' && this.state.scrollY._value > 80) {
				Animated.timing(this.state.barTranslate, {
					toValue: 0,
					duration: 300
				}).start();
		...
		Animated.event([ { nativeEvent: { contentOffset: { y: this.state.scrollY } } } ])(e);
	}

	camelize(str) {
		if (str && typeof str === 'string') {
			str = str.trim().toLowerCase().replace(/([^A-Za-z\u00C0-\u017F0-9]+)(.)/gi, function() {
			...
			<View style={styles.backgroundBody}>
				<Animated.View
					style={{
						...
						transform: [ { scale: this._scaleBG }, { translateY: this._translateBG } ]
					}}
				>
					{station &&
					station.img && (
						<ImageCache source={{ uri: station.img['bg-low'] }} style={styles.stationBackground} />
					)}
					<LinearGradient
	...
	renderName() {
		...
		return (
			<View style={styles.stationNameContainer} accessible={true}>
				<Animated.Text style={[ styles.stationLabel, { opacity: stationNameOpacity } ]}>ESTAÇÃO</Animated.Text>
				<Animated.Text style={[ styles.stationName, { opacity: hideStationName, fontSize: stationFontSize } ]}>
					{station.name}
				</Animated.Text>
			</View>
		);
		...
		return (
			<View>
				<Header
					...
					style={styles.header}
				/>
				<ScrollView onScroll={this.onScroll.bind(this)} scrollEventThrottle={16} style={styles.scrollContent}>
