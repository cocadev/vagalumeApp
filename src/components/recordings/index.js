...
class RecordingsPage extends Component {
	...
	filterDate(ts) {
		...
		return (
			<View style={{ alignItems: 'center', flexDirection: 'row' }}>
				<Text style={{ fontFamily: 'rubik', color: '#AAA', fontSize: 14 }}>{formattedDay}</Text>
				<View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 8 }}>
					<Icon name="clock" size={18} color="#AAA" />
					<Text style={{ fontFamily: 'rubik', color: '#AAA', fontSize: 14 }}>{formattedHour}</Text>
				</View>
			</View>
		);
	}
	...
	enableEdit() {
		...
		Animated.sequence([
			Animated.timing(this.state.recordingX, {
				toValue: 50,
				duration: 200
			})
		]).start();
	...
	pressRecording(recordingID) {
		...
		if (castSessionResumed || castSessionStarted || castSessionConnected) {
			Toast.show(
				'Desculpe! Ainda não é possível executar uma gravação no Chromecast. Por favor, desconecte-se do Chromecast para reproduzir a gravação.'
			);
			return;
		}
	...
	deleteRecordings() {
		Alert.alert('Excluir gravações?', 'Tem certeza que deseja excluir as gravações selecionadas?', [
			{
				text: 'Sim',
				onPress: () => {
					const newRecordings = [];
					let recordingLive;
					const currentRecording = this.state.recordings.find(
						(obj, key) => key === this.state.indexPlayingRecording
					);
					...
					for (let i = 0; i < this.state.recordings.length; i++) {
						...
						if (
							...
							recording.id == this.props.recordingFile.id
						) {
					...
					if (recordingLive) {
						this.props.initPlayer({
							playerInfo: recordingLive.station,
							playerType: PLAYER_TYPES.STREAM,
							autoplay: true
						});
					}
			...
			return (
				<ImageCache
					...
					style={{
						...
						height: 60,
						borderRadius: 30,
						borderColor: this.state.indexPlayingRecording === key ? '#CF0' : '#333',
						borderWidth: 1
					}}
				/>
			);
	...
	renderRecordingName(recording) {
		if (recording && recording.station && recording.station.name) {
			...
			return <Text style={{ fontFamily: 'rubik-medium', color: '#FFF', fontSize: 14 }}>{name}</Text>;
		} else {
			return <View />;
		}
	}
						...
						<View key={key}>
							...
							>
								<View
									style={{
										backgroundColor:
											isSelected > -1 || this.state.indexPlayingRecording === key
												? 'rgba(0, 0, 0, 0.1)'
												: '#2d2d2d',
										paddingLeft: 16,
										paddingRight: 16
								...
								>
									<Animated.View
										style={{
											opacity: this._selectRecordingOpacity,
											height: 20,
											width: 20,
											position: 'absolute',
											top: 35,
									...
									>
										...
										{isSelected > -1 && (
											<View style={{ borderRadius: 20, position: 'absolute' }}>
												<Image
													style={{ width: 20, height: 20 }}
													source={require('../../img/ic_check_verde.png')}
												/>
											</View>
										)}
									</Animated.View>

									<View style={{ borderTopColor: '#3d3d3d', borderTopWidth: key === 0 ? 0 : 1 }} />

									<Animated.View
										style={{
											transform: [ { translateX: this.state.recordingX } ],
											height: isSelected > -1 ? this.state.recordingHeight : 90,
											alignItems: 'center',
											flexDirection: 'row'
										}}
									>
			...
			return (
				<View style={{ backgroundColor: '#2d2d2d', paddingLeft: 16, paddingRight: 16 }} accessible={true}>
					<View style={{ height: 90, alignItems: 'center', flexDirection: 'row' }}>
						<View style={{ width: 60, height: 60, borderRadius: 30 }}>
							{playerInfo.img &&
							playerInfo.img.default && (
								<ImageCache
									source={{ uri: playerInfo.img.default }}
									style={{
										width: 60,
										height: 60,
										borderRadius: 30,
						...
						<View style={{ marginLeft: 16 }}>
							...
							<View style={{ flexDirection: 'row', alignItems: 'center' }}>
								...
								<RecorderTimer
									styles={{ fontFamily: 'rubik', color: '#AAA', fontSize: 14 }}
									recordingTS={isRecording}
								/>
							</View>
							{isRecording ? (
								<View style={{ flexDirection: 'row', alignItems: 'center' }}>
									<Animated.View
										style={{
											opacity: this.state.recordingOpacity,
											height: 10,
											width: 10,
				...
				<View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
					<View style={{ marginBottom: 40, justifyContent: 'center', alignItems: 'center' }}>
						...
						<View style={{ justifyContent: 'center', alignItems: 'center' }}>
							...
							>
								<Text style={{ backgroundColor: 'transparent', color: '#CF0' }}>Grave</Text> as estações
							</Text>
							<Text
								style={{
									backgroundColor: 'transparent',
							...
							>
								para ouvir{' '}
								<Text style={{ backgroundColor: 'transparent', color: '#CF0' }}>offline</Text> sem
							</Text>
							<Text
								style={{
									backgroundColor: 'transparent',
		...
		return (
			<View style={{ backgroundColor: '#2d2d2d', flex: 1 }}>
				<Prompt
					...
					defaultValue={
						this.state.renameItem ? this.state.renameItem.name || this.state.renameItem.station.name : ''
					}
