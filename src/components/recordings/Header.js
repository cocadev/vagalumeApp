	...
	renderEditButton() {
		...
		return (
			...
			>
				<View style={{ marginRight: 16 }}>
					<Text style={styles.editText}>
						{recordings instanceof Array && recordings && recordings.length > 0 && (editMode ? 'Cancelar' : 'Editar')}
					</Text>
				</View>
			</TouchableOpacity>
		);
	...
	render() {
		...
		return (
			<View style={styles.body}>
                ...
                <Text style={styles.title}>{selectedRecordings.length || (editMode ? 'Selecionar...' : 'Gravações')}</Text>
