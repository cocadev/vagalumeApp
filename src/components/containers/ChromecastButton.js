...
class ChromecastButton extends Component {
	...
	componentWillReceiveProps(nextProps) {
		if (this.props != nextProps) {
			...
			if(((nextProps.cast.session == 'onRouteAdded') && (!this.state.chromecastAvailable)) ||
			    ((nextProps.cast.session == 'onRouteRemoved') && (this.state.chromecastAvailable))) {
				this.getAvailableDevices();

			} else if (nextProps.cast.session === 'onSessionStarting' ||  nextProps.cast.session === 'onSessionResuming') {
				this.opacity();

			} else if (nextProps.cast.session === 'onSessionEnded' || nextProps.cast.session === 'onSessionResumeFailed' || nextProps.cast.session === 'onSessionSuspended' ) {
				this.resetOpacity();
	...
	opacity() {
		...
		Animated.timing(
			...
			if(this.state.chromecastSession === 'onSessionStarting' ||  this.state.chromecastSession === 'onSessionResuming'){
			 	this.opacity();
			}
		}
		);
