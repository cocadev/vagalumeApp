...
class Lyrics extends Component {
				...
				<TouchableOpacity onPress={this.showTranslation.bind(this, true)}>
					<View
						style={{
							...
							paddingLeft: 10,
							borderRadius: 20,
							backgroundColor: showTranslation ? 'rgba(255, 255, 255, 0.2)' : 'transparent'
						}}
					>
						<Text style={{ color: '#FFF', fontSize: 14, fontFamily: 'rubik' }}>TRADUÇÃO</Text>
					</View>
				</TouchableOpacity>
			);
		}
	...
	renderTabs() {
		...
		if (data && data.translate && data.translate.length) {
			return (
				<View
					style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'row', marginBottom: 16 }}
				>
					<TouchableOpacity onPress={this.showTranslation.bind(this, false)}>
						<View
							style={{
								...
								paddingLeft: 10,
								borderRadius: 20,
								backgroundColor: showTranslation ? 'transparent' : 'rgba(255, 255, 255, 0.2)',
								marginRight: 16
							}}
						>
							<Text style={{ color: '#FFF', fontSize: 14, fontFamily: 'rubik' }}>ORIGINAL</Text>
						</View>
					</TouchableOpacity>
		...
		return (
			<Animated.View
				style={[
					...
					{ zIndex: 2, opacity: this._fadeLyrics, paddingTop: 90, paddingLeft: 16, paddingRight: 16 }
				]}
			>
				{this.renderTabs()}
				<ScrollView
