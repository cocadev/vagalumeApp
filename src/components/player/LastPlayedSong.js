...
class LastPlayedSong extends Component {
	...
	render() {
		...
		const lastPlayedImage = song && song.img && song.img.medium ? { uri: song.img.medium } : placeholderImage;
		...
		return (
			<View style={styles.body} accessible={true}>
					...
					<View style={styles.imageContainer}>
						<View style={{ width: 55, height: 55, backgroundColor: 'transparent' }}>
							<ImageCache
								...
								borderRadius={27.5}
								style={{ borderRadius: 27.5, width: 55, height: 55, position: 'absolute' }}
							/>
				...
				<View style={styles.textContainer}>
					...
					<Text numberOfLines={1} ellipsizeMode="tail" style={[ styles.text, { color: '#FFF' } ]}>
						{song.title.name}
					</Text>
